<script src="../../assets/js/area.js"></script>
<template>
  <div>
    <van-nav-bar
        title="新增地址"
        left-arrow
        @click-left="onClickLeft"
    />
    <van-address-edit
        :area-list="areaList"
        :address-info="addressInfo"
        show-set-default
        show-delete
        :area-columns-placeholder="['请选择', '请选择', '请选择']"
        @save="onSave"
        @delete="onDelete"
    />
  </div>
</template>

<script>
import areaList from "../../assets/js/area.js";
import { Toast,AddressEdit,NavBar } from 'vant';

export default {
  components: {
    [Toast.name]: Toast,
    [AddressEdit.name]: AddressEdit,
    [NavBar.name]: NavBar
  },
  created() {
    this.initData();
  },
  data() {
    return {
      searchResult: [],
      areaList,
      addressId:'',
      listData:[],
      addressInfo:{},
      isDefault:false
    };
  },
  methods: {
    initData(){
      console.log("我在请求初始数据！")
      this.addressId = this.$route.query.addressId;
      console.log("addressId:"+this.$route.query.addressId);
      this.$axios({
        method: "get",
        url: 'http://localhost:8003/address',
        params:{
          addressId: this.addressId
        }
      })
          .then(res => {
            this.listData = res.data.data;
            if(this.listData.defaultStatus == 1){
              this.isDefault = true;
            }
            this.addressInfo = {
              name :this.listData.name,
              tel :this.listData.tel,
              province : this.listData.province,
              city :this.listData.city,
              country :this.listData.region,
              addressDetail :this.listData.detailAddress,
              areaCode :this.listData.post,
              isDefault:this.isDefault
            };
            console.log("adin:"+this.addressInfo.name);
          });
    },
    onDelete() {
     // Toast('delete');
      this.$axios({
        method: "delete",
        url: 'http://localhost:8003/address',
        params:{
          addressId: this.addressId
        }
      })
          .then(res => {
            console.log(res);
            Toast("删除成功！");
            this.$router.push('/address');
          })
          .catch(error => {
            window.console.log(error);
          });
    },
    onSave(content) {
      this.name=content.name;
      this.phone=content.tel;
      this.province=content.province;
      this.city=content.city;
      this.region=content.county;
      this.detail_addr=content.addressDetail;
      this.group_message=content.isDefault;
      this.areaCode=content.areaCode;
      console.log(this.areaCode);
      console.log(this.city);
      console.log(this.province);
      this.$axios({
        method: "put",
        dataType: "json",
        url: "http://localhost:8003/address",
        params: {
          name: this.name,
          tel:this.phone,
          province:this.province,
          city:this.city,
          region: this.region,
          detailAddress:this.detail_addr,
          defaultStatus:this.group_message,
          post:this.areaCode,
          addressId: this.addressId
        }
      })
          .then(res => {
            console.log("返回的数据 =", res);
            Toast('已修改地址！');
            this.$router.push('/address');
          })
          .catch(error => {
            console.log(error);
          });
    },

  },
};
</script>
